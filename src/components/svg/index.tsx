...
function SvgElement({ onClick, name, size = 33, margin, noColor }: SectionType) {
    ...
    useEffect(() => {
        const find_one = icons.find(s => s.name === name)
    ...
    return (
        ...
        }} onClick={onClick ? onClick : undefined} className={`${noColor ? "" : colors.fa_secondary} ${hover ? styles.hover : ""} ${pointer ? styles.pointer : ""} ${className ? className : ""}` } xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${svg.w} ${svg.h}`}>
            {
